    ...
    uint8_t *hmac_keydata, uint8_t *iv, uint8_t *mac, zio_crypt_key_t *key)
{
	...
	plain_iovecs[1].iov_base = (char *)key->zk_hmac_keydata;
	plain_iovecs[1].iov_len = HMAC_SHA256_KEYLEN;
	plain_iovecs[2].iov_base = (char *)outmac;
    ...
    uint_t *enc_len)
{
	...
	for (i = 0; i < max_dnp; i += sdnp[i].dn_extra_slots + 1) {
		...
		if (dnp->dn_type != DMU_OT_NONE &&
			...
			src_iovecs[nr_iovecs].iov_base = (void *)bonus;
			src_iovecs[nr_iovecs].iov_len = crypt_len;
			dst_iovecs[nr_iovecs].iov_base = (void *)DN_BONUS(&ddnp[i]);
			dst_iovecs[nr_iovecs].iov_len = crypt_len;
    ...
    uio_t *puio, uio_t *cuio, uint_t *enc_len)
{
	...
	if (!encrypt) {
		...
		mac_out_iov->iov_base = (void *)out_mac;
