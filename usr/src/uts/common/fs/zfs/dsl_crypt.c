    ...
    dsl_crypto_params_t **dcp_out)
{
	...
	/* if the user asked for the default crypt, decide that now */
    ...
    uint64_t dckobj, void *tag, dsl_crypto_key_t **dck_out)
{
	...
	dck->dck_wkey = wkey;
	dck->dck_obj = dckobj;
	(void) refcount_add(&dck->dck_refcnt, tag);
    ...
    dsl_crypto_key_t **dck_out)
{
	...
	/* increment the refcount */
	(void) refcount_add(&found_dck->dck_refcnt, tag);
    ...
    dsl_crypto_key_t **dck_out)
{
	...
	if (found_km && tag)
		(void) refcount_add(&found_km->km_key->dck_refcnt, tag);
